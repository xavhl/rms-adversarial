...
def project_perturbation(epsilon, p, perturbation):

    if p == 2:
        perturbation = perturbation * min(1, epsilon / np.linalg.norm(perturbation.flatten(1)))
    elif p == np.inf:
        # perturbation = np.sign(perturbation) * np.minimum(abs(perturbation), epsilon) # [-eps, +eps]
        perturbation = np.maximum(np.minimum(perturbation, -epsilon), 0) # [-eps, 0] # only allow negative perturbation for darker colors
    return perturbation

def get_image_size(model):
    if isinstance(model, models.resnet152()):
        return 224
    elif isinstance(model, models.resnet152()):
        return 299

def generate(accuracy,trainset, testset, net, delta=0.2, max_iter_uni=np.inf, xi=10, p=np.inf, num_classes=1000, overshoot=0.2, max_iter_df=20):
    '''
